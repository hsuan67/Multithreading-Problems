	...
	public void run() {
		synchronized (queue) {
			while (queue.size() >= max) {	// call wait to wait
				try {
					System.out.println("Producer" + Thread.currentThread().getName() + " start waiting...Queue reach max capacity");
					queue.wait();
					// thread執行到這就代表等待結束
			...
			System.out.println("Producer" + Thread.currentThread().getName() + " produce Product" + String.valueOf(ran_int));
