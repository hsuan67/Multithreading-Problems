	...
	public void run() {
		synchronized(queue) {
			while(queue.isEmpty()) {	//等待到拿鎖到鎖之間可能有其他thread先拿到鎖 queue可能又空了 所以要再判斷一次
				try {
					System.out.println("Consumer" + Thread.currentThread().getName() + " start waiting...Queue is empty");
			...
			System.out.println("Consumer" + Thread.currentThread().getName() + " consume " +product.getName());
